globals [
  percent-happy   ;; variables ilustrées dans nos graphes en pourcantage 
  percent-density
  percent-other
]
...
to setup
  ...
  ;; création aléatoir de turtles rouges sur les patches
  ask n-of number-red-resident patches
    [ sprout 1
      [ set color red ] ]
  ;; création aléatoir de turtles yellow sur les patches
   ask n-of number-yellow-activity patches
  [sprout 1
  [set color yellow] ]
;; création aléatoir de turtles white sur les patches
...
to go
  if percent-happy = 100  [ stop ] ;; le programme s'arrète lorsque on atteint un niveau de satisfaction 
...
to move-unhappy-turtles ;; lorsques les turtles ne sont pas happy, elles bougent aléatoirement sur un autre patch 
...
to find-new-spot ;; fonction pour trouver un nouveau patch aléatoirement avec leurs degrés de libertés
  ...
  if any? other turtles-here
    [ find-new-spot ]  ;; seulement si le patch n'est pas déja occupé
  move-to patch-here 
end
...
to update-turtles
  ...
  ask turtles with [color = red] [  ;;  par rapport aux résidents ( rouge )
    ...
    ;; condition pour que les résidents soient happy ( une attraction avec les activités mais une répulsion des industries)
  ]     
    ...
    set happy? similar-nearby >= (industry-wants-industry) and industry-accepts-resident >= ( red-nearby )
     ;; condition pour que les industries soient happy ( une attraction avec les industries mais une répulsion des résidents)
  ] 
    ...
    ;; condition pour que les activités soient happy ( une attraction avec les industries et une attraction avec les résidents)
  ]  
     
      ask turtles [
    ;; in next two lines, we use "neighbors" to test the eight patches
    ;; surrounding the current patch
    set other-nearby count (turtles in-radius 3)
      with [color != [color] of myself]
      set total-nearby count (turtles in-radius 3)
  ]  
...
"" ""
PENS
"percent" 1.0 0 -16777216 true "" "plot percent-other"
